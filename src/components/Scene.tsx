 ...
 * traps the user in several separate scroll areas) and NOT by a static
 * negative-margin overlap (that permanently hides whatever content falls
 * in the overlapped band, regardless of scroll position — the bug this
 * replaced). Instead: the content is translated upward by GSAP in lockstep
 * with scroll, over a dedicated "runway" of extra scroll distance sized to
 * exactly that excess height. By the time the runway ends, the section's
 * own bottom has scrolled fully into view — only then does the next
 * scene's hold begin rising over it.
 */
